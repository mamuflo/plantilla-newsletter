...
def save_template():
    ...
    form_data = session.get('form_data')
    if not form_data:
        return jsonify({'error': 'No form data found to save'}), 400
    ...
    with tempfile.NamedTemporaryFile(mode='w+', delete=False, encoding='utf-8') as temp_file:
        json.dump(form_data, temp_file, ensure_ascii=False, indent=4)
        temp_file_path = temp_file.name
    ...
    try:
        ...
        request_drive = drive_service.files().create(media_body=media, body=file_metadata, fields='id')
        response = request_drive.execute()
    except Exception as e:
    ...
    if response:
        return jsonify({'success': True, 'fileId': response.get('id')})
    else:
        return jsonify({'error': 'Failed to save template'}), 500

@app.route('/list_templates_in_folder/<folder_id>', methods=['GET'])
def list_templates_in_folder(folder_id):
    if 'credentials' not in session:
        return jsonify({'error': 'Not authenticated'}), 401
    ...
    try:
        ...
        print("Error listing templates: {}".format(e))
        return jsonify({'error': 'Failed to list templates'}), 500

@app.route('/load_template/<file_id>', methods=['GET'])
def load_template(file_id):
    ...
    try:
        ...
        downloader = MediaIoBaseDownload(file_content, request_drive)
        done = False
        while done is False:
            status, done = downloader.next_chunk()
        
        file_content.seek(0)
        template_data = json.load(io.TextIOWrapper(file_content, encoding='utf-8'))
        ...
        print("Error loading template: {}".format(e))
        return jsonify({'error': str(e)}), 500
